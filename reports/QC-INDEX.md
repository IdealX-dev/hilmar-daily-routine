...
Generated 2026-05-13, updated 2026-07-26. Total active checks: **71**
(QC-001 through QC-074, including the QC-014a/b and QC-020a/b sub-variants;
...
invariants, 2026-07-10), QC-066..QC-072 (the 2026-07-23..26 data audit:
impossible states, open-RFQ-as-loss, OL SLA, duplicate rows, TEU sanity
ceiling, day-bucket clock, history/status contradiction, standalone
booking hygiene, win-evidence consistency); see the matrix below.
...
| QC-006 | WARN | Suspiciously-large TEU on a single row (>30 `teu_requested`) — the ADVISORY band: unusual for Hilmar but possibly real, so it asks for a human eyeball. **Not** the poisoned-data guard: on 2026-07-26 it fired correctly on the 89,028 TEU row and the report shipped anyway, because a WARN neither gates nor heals and this only ever looked at `teu_requested`. See **QC-070** for the hard ceiling. | None | 2026-04 baseline |
...
| QC-068 | **WARN** | **OL response-SLA breach** — an open `PENDING_OL` row where OL has not quoted within **3 BUSINESS hours** (ET 8:30–17:30 Mon–Fri), per Michael 2026-07-26 "ol response time has to be 3 hours" (`config.json rules.overdue_no_response_hours`). Measured with `core.pending_ol_overdue` → `biz_hours_between`, the SAME clock as the report's Time-to-Quote column, so the SLA and the displayed wait can never disagree; nights and weekends never count against OL. NOT a data defect and NOT a loss threshold — the rows are correctly stored open, and they stay open until the 24/72h win-loss timer resolves them. | **No heal** — only OL sending the quote clears it. The audit names every lane and its business-hours wait so the OL desk gets chased the same morning; the daily email flags the same rows red ⚠ in "Pending OL Quote". | 2026-07-26 |
| QC-069 | **ERROR** | **One shipment stored as two rows** (operator-reported defect #2 — a lane showing as won AND still pending in the same report). Two shapes: (a) one `mdolx_ref` on more than one row — `link_bookings_to_requests` emits a standalone `stand_<mdolx>` WIN when it cannot link a booking to its request; (b) an open PENDING row shadowed by a WIN row on the same destination + container spec, where the win landed at or after the ask. Destination matching is ALIAS-AWARE (`HCMC (Cat Lai)` ≡ `HCMC` ≡ `Cat Lai`) because `canonical_lane_key` is only `.lower()`, which is precisely how OL's "Cat Lai" confirmation fails to link to Lonny's "HCMC" RFQ. Impact: TEU double counted, lane win-rate diluted, and the orphaned PENDING copy later ages into a LOSS claiming OL never quoted a move OL actually booked. | **Detect-only** — collapsing rows is destructive and the correct survivor depends on which row carries the real request thread. The audit names BOTH request_ids so the operator (or a later, evidenced heal) can fold them. | 2026-07-26 |
| QC-070 | **ERROR** | **TEU that cannot be real** — a per-row volume above the `core.MAX_ROW_TEU` (100) / `MAX_ROW_CONTAINERS` (60) ceiling, or a `containers` string that plainly names equipment yet recomputes to 0 TEU. Root cause 2026-07-26: a reference number in a subject line ("PO 4451440") parsed as 44,514 x 40' = **89,028 TEU on ONE row**, and because every volume figure in the email, dashboard, PDF and lane rollup is a SUM over rows, that single row silently rewrote the whole day's numbers. `core.parse_teu`'s regex was hardened the same day; this check is the second line of defence that makes the regex non-load-bearing — `parse_teu` now REFUSES any parse above the ceiling (returns 0 rather than the poisoned figure), and QC-070 errors on any stored row above it whoever wrote it (older build, carry-forward, hand edit). Ceiling calibration: largest real Hilmar sample on record is 6x40'RF = 12 TEU, so 100 TEU is ~8x beyond anything the business has ever asked for — a real quote can never trip it. **Why QC-006 was not enough:** QC-006 (>30 TEU) DID fire on the 89,028 row and the report shipped regardless — it is a WARN, it neither gates nor heals, and it only ever inspected `teu_requested`. QC-070 is ERROR-class, heals, and covers `teu_won` and `container_count` too. Both stay: "unusually large" and "impossible" are different questions. | **SELF-HEAL for over-counts** — `teu_requested` / `teu_won` / `container_count` are recomputed from the row's OWN `containers` text via `parse_teu`, so a heal can never write a value ingest would itself refuse (`teu_won` clears to 0 on a non-WIN row, since it is win evidence). **Detect-only for the 0-TEU shape** — that is either a parser gap or a refused parse, and healing it would mean INVENTING a volume, which is the exact class of guess that caused the defect. | 2026-07-26 |
| QC-071 | **ERROR** | **Day buckets on the wrong clock** — a row whose `request_date` is not the ET calendar date of its own `request_timestamp`. Root cause 2026-07-26: `request_date` had THREE producers writing THREE clocks — `ingest` wrote the UTC calendar date, `merge_ingest` took a raw `ts[:10]` UTC slice, and this file's own heal wrote PT — while every reader (gen_email, gen_dashboard, gen_client_email, the day reconciliation) buckets by the ET business day from `core.report_business_day`. An RFQ Lonny sent Friday 5:30 PM PT is `2026-07-25` in UTC and Friday `2026-07-24` in ET; since no fire ever reports a Saturday, that row appeared in **NO** day's New Requests, KPI tile or day reconciliation — on any day, ever — while still counting toward the period totals, so the day tiles and period tiles disagreed by exactly the rows the clocks disagreed about, and the day reconciliation still balanced because the row was never in the denominator. All three producers now call `core.et_date_of`. | **SELF-HEAL in phase_3** — `request_date` is RECOMPUTED from `request_timestamp` every pass (not merely filled when absent), which is also the MIGRATION for every row already stored on the wrong clock; filling-only would have left them mis-dated forever. The legacy `date` mirror is kept in step, since readers fall back to it. Rows with no parseable timestamp are left alone — we correct dates, we don't invent them. Reaching QC-071 means the heal did not reach the row, so the check itself is detect-only. | 2026-07-26 |
| QC-072 | **ERROR** | **A row contradicts its own audit trail** — `status_history[-1]["to"]` disagrees with `status`, or `teu_won > 0` on a row that is not a WIN. `status_history` is the field `schema.json` declares as THE transition record, so audits, the dashboard timeline and Sentry triage all reconstruct a row's outcome from it. Root cause 2026-07-26: two writers bypassed it — `age_requests` assigned `r["status"]` directly, and `merge_idempotent` recomputed `status` (it is in `_RECOMPUTED_FIELDS`) while PRESERVING the old history (it is not). A send-signal WIN that never booked therefore read status=LOSS / SEND_NO_BOOKING with history still ending at `{"to": "WIN"}` — every history-based reader called it WON, with no entry anywhere explaining the regression — and carried `teu_won=2` for a shipment that was never booked. Both writers now route through `core.record_transition`; the merge UNIONS the log (collapsing consecutive re-derived duplicates, so it cannot grow every fire) instead of keeping the stale copy. | **Detect-only** — rewriting history is never a safe automatic act; the audit names the row so a human decides which record is true. The `teu_won` half IS prevented at the source: leaving WIN clears the booked volume (`_clear_win_evidence_on_exit`), deliberately narrow — only the volume, never the `has_send` / `mdolx_ref` evidence. | 2026-07-26 |
| QC-073 | **ERROR**/WARN | **Fabricated or degenerate values on a standalone booking row.** A `stand_<mdolx>` row is what `link_bookings_to_requests` writes when it cannot find the RFQ a booking belongs to — synthesised from ONE subject line, so it is the likeliest place for invented data to enter the dataset (Michael's reported defect #3: degenerate lanes and blank carrier/vessel/dates shown as real shipments). Three shapes: **(a) ERROR** origin and destination resolve to the same port via `core.canonical_port_key`, so `Oakland → Oakland` renders in Lane Performance as a real trade lane — happens on re-forwarded / return-leg confirmations naming only one port; **(b) ERROR** a `response_timestamp` on a `stand_` row with no `ol_rate` — a booking confirmation is NOT a rate quote, and writing the booking time there makes the row claim an OL response that never happened, corrupting every turnaround metric it feeds (the same 171-hour defect fixed on the MATCHED path in 2026-05-19 and left live on this one); **(c) WARN** a standalone WIN with no `carrier_won` — not corruption, but an unattributable win the operator must go find the thread for. | **Detect-only** — these are real bookings; the fix is to link them to their request or correct the source subject, and both need human eyes. (a) and (b) are now PREVENTED at the source in the standalone constructor, so reaching the check is a regression. | 2026-07-26 |
| QC-074 | **ERROR**/WARN | **A row's WIN evidence and its outcome disagree.** Three shapes. **(a) ERROR — duplicate `request_id`:** the additive carry-forward used to APPEND a prior WIN beside the row the fresh stage had already rebuilt under the SAME id (the RFQ email is still inside the stage window, so the fresh build reconstructs it as PENDING with no knowledge of the booking). `tracking-data-v2.json` then reported 2 entries and 8 TEU for one 4-TEU shipment, the id simultaneously PENDING/NQ and WIN — and `phase_4` arbitrated by counting non-empty FIELDS, in one observed run discarding the very win the carry-forward exists to protect (MDOLX260500 Oakland→Yokohama, stamped WIN from Linda's recap). **(b) ERROR — booked but not won:** a row carrying an `mdolx_ref` that is not a WIN and is not explicitly held as `MDOLX_NO_SEND`; a booking ref is hard evidence OL moved the shipment, so reporting it as a loss tells the client we lost a move OL booked. **(c) WARN — won without evidence:** a WIN with neither `mdolx_ref` nor `has_send`. | **Detect-only** — which row is the true one depends on evidence outside the dataset, so the audit names the ids for a human. (a) is PREVENTED at the source: the carry-forward now reconciles by `request_id` and MERGES the prior win's evidence into the rebuilt row (`_merge_prior_win_into`), recording the transition so QC-072's invariant holds. A status contradiction is never resolved by counting fields — an mdolx-backed WIN is evidence, a rebuilt PENDING is the absence of it. | 2026-07-27 |
